...
class PositionalEncoding(nn.Module):
    def __init__(self, max_len, embed_dim, required_grad=False):
        ...
        self.register_buffer('encodings', self._build_positional_encodings())
    ...
    def _build_positional_encodings(self):
        ...
        div_term = torch.exp(torch.arange(0, self.embed_dim, 2, dtype=torch.float32) * -(torch.log(torch.tensor(10000.0)) / self.embed_dim))

        encoding[:, 0::2] = torch.sin(position_idx * div_term)
        encoding[:, 1::2] = torch.cos(position_idx * div_term)

        return encoding
    
    def forward(self, x):
        seq_len = x.shape[1]
        x = x + self.encodings[:seq_len, :]
        return x
...
class Attention(nn.Module):
    ...
    def forward(self, src, tgt=None, attention_mask=None, causal=False):
        ...
        if tgt is None:
            ...
            # Decoder Self-Attention applies causality, cross-attention does NOT.
            # If causal=True and attention_mask is present, we must let attn_mask handle it
            # or pass None to is_causal if an explicit mask is provided.
            attention_out = F.scaled_dot_product_attention(
                q, k, v,
                attn_mask=attention_mask,
                dropout_p=self.config.attention_dropout_p if self.training else 0.0,
                is_causal=causal if attention_mask is None else False
            )
...
class TransformerDecoderlayer(nn.Module):
    ...
    def forward(self, src, tgt, src_mask = None, tgt_mask = None):
        tgt = tgt + self.dec_attn_dropout(self.dec_attn(tgt, attention_mask=None, causal=True))
...
if __name__ == "__main__":
    ...
    print("Inference Test Complete. Token IDs Output:", out)