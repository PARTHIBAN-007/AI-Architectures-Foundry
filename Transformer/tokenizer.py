...
def train_tokenizer(path_to_data_root):
    ...
    french_files = glob.glob(os.path.join(path_to_data_root, "**/*.fr"), recursive=True)
    
    if not french_files:
        raise FileNotFoundError(f"No .fr files found in {path_to_data_root}. Check your path!")

    print(f"Found {len(french_files)} files for training.")
